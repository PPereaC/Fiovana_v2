...
namespace Fiovana_v2.OpenIddict;
...
public class OpenIddictDataSeedContributor : OpenIddictDataSeedContributorBase, IDataSeedContributor, ITransientDependency
{
    ...
    private async Task CreateScopesAsync()
    {
        ...
        {
            Name = "Fiovana_v2", 
            DisplayName = "Fiovana_v2 API", 
            Resources = { "Fiovana_v2" }
        });
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        var consoleAndAngularClientId = configurationSection["Fiovana_v2_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var consoleAndAngularClientRootUrl = configurationSection["Fiovana_v2_App:RootUrl"]?.TrimEnd('/');
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["Fiovana_v2_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["Fiovana_v2_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        // Web Public Client
        var webPublicClientId = configurationSection["Fiovana_v2_Web_Public:ClientId"];
        if (!webPublicClientId.IsNullOrWhiteSpace())
        {
            var webPublicRootUrl = configurationSection["Fiovana_v2_Web_Public:RootUrl"]!.EnsureEndsWith('/');
            ...
            await CreateOrUpdateApplicationAsync(
                ...
                secret: configurationSection["Fiovana_v2_Web_Public:ClientSecret"] ?? "1q2w3e*",
